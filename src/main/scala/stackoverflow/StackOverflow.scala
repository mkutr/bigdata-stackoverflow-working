...
//    val groupedPostings: RDD[(String, Iterable[Posting])] = postings.groupBy{ p => if (p.postingType == 1) "question" else "answer"}


    val questions: RDD[(QID, Question)] = postings.filter(_.postingType == 1).map(p => (p.id, p))
    val answers: RDD[(QID, Answer)] = postings.filter(_.postingType == 2).map(p => (p.parentId, p)).collect{ case (Some(qid), posting) => (qid, posting)}
//    val answers: RDD[Iterable[(QID, Answer)]] = postings.filter(_.id == 2)
//      .map(p => (p.parentId, p))
//      .groupBy(_._1)
//      .collect{ case (Some(qid), it) => (qid, it.map(_._2))}
//      .map(el => el._2.map((el._1, _)))
//


    val ans: RDD[(QID, (Question, Answer))] =
  ...
  def scoredPostings(grouped: RDD[(QID, Iterable[(Question, Answer)])]): RDD[(Question, HighScore)] =
    ...
    grouped.map{case (qid, l) => (l.head._1, answerHighScore(l.map(x => x._2).toArray))}
  ...
  def vectorPostings(scored: RDD[(Question, HighScore)]): RDD[(LangIndex, HighScore)] =
    ...
    val x: RDD[(Option[LangIndex], HighScore)] = scored.map{ case (question, score) => (firstLangInTag(question.tags, langs), score)}
    x.collect{case (Some(langindx), score) => (langindx * langSpread, score)}
  ...
  @tailrec final def kmeans(means: Array[(Int, Int)], vectors: RDD[(Int, Int)], iter: Int = 1, debug: Boolean = false): Array[(Int, Int)] =
    ...
    val clustered: RDD[((Int, Int), (Int, Int))] = vectors.map(v => (findClosest(v, means), v))
    val clusterAvg: Map[(Int, Int), (Int, Int)] = clustered.groupByKey().mapValues(averageVectors).collect().toMap // mean, avg
    val newMeans = means.map(m => clusterAvg(m))
